...
static int
GetNISTHeader(Sound *s, Tcl_Interp *interp, Tcl_Channel ch, Tcl_Obj *obj,
	      char *buf);

static char * 
GuessNISTFile(char *buf, int len)
{
  if (len < strlen(NIST_STRING)) return(QUE_STRING);
...
static int
OpenNISTFile(Sound *s, Tcl_Interp *interp, Tcl_Channel *ch, char *mode)
{
   /* open sphere file  */
  *ch = (Tcl_Channel) sp_open(Snack_GetSoundFilename(s), mode);
  if (*ch == NULL)
    fail("SPHERE: unable to open file");

  /*
    Hack for the pculaw format. Read header again because
    sp_set_data_mode() has to be called.
    */
  GetNISTHeader(s, interp, *ch, NULL, NULL);

  return TCL_OK;
}

static int
CloseNISTFile(Sound *s, Tcl_Interp *interp, Tcl_Channel *ch)
{
  /* close */
...
static int
ReadNISTSamples(Sound *s, Tcl_Interp *interp, Tcl_Channel ch, char *ibuf, char *obuf, int len)
{
...
static int
SeekNISTFile(Sound *s, Tcl_Interp *interp, Tcl_Channel ch, int pos)
{
  /* seek to pos */
  if (sp_seek((SP_FILE *)ch, (pos-NIST_HEADERSIZE)/
	      ...
	      char *buf)
{
   ...
   if (Snack_GetDebugFlag(s)) Snack_WriteLog("\tReading NIST header\n");
   ...
   Snack_SetHeaderSize(s, NIST_HEADERSIZE);
...
EXPORT(int, Snacksphere_Init) _ANSI_ARGS_((Tcl_Interp *interp))
{
  ...
  res = Tcl_PkgProvide( interp, "snackSphere", "1.0");
  
  if (res != TCL_OK) return res;
  
  return Snack_AddFileFormat(NIST_STRING, GuessNISTFile, GetNISTHeader,
			     NULL, NULL,
			     OpenNISTFile, CloseNISTFile, ReadNISTSamples,
			     NULL, SeekNISTFile);
}
