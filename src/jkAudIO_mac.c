/*
  ...
  This is just a raw update/reformatting of original code supplied by Dan Ellis.
  Probably won't work as is. It is distributed with the hope that some brave
  person will compile and hopefully get it to work without too much problem.
  Please contact Kare Sjolander (kare@speech.kth.se) for any questions that
  arises in this process.

  You need to define MAC_SNACK. Also, add dpwe_compat.c to the project.

 */

/*
 * Copyright (C) 1999 Dan Ellis
 *
...
/* #define DEBUG 1 /* do we write dbg msgs to dbg.out? */
...
#define DBGSTR(a) dbgStr(a)
#define DBGINT(n) dbgInt(n)
...
/* init support */
static int GetSndInputNames(long inRefNum, int which, char *buf) 
{
...
static void dbpFill(ADesc *A)
{   /* using the data pointers in the au, fill in as much more into the current db.
       Carry on and fill the other if it is not already full. */
    char  *src, *dst;
...
static pascal void DoubleBackProc(SndChannelPtr /* schn */, SndDoubleBufferPtr db)
...
static int MySndIdle(SndChannelPtr schn)
...
static void AUWaitNoData(ADesc *A)
...
static void AUWaitNotBusy(ADesc *A)
{   /* block until the current playing is complete */
    ...
    DBGSTR("AUWaitNotBusy\n");
    while(MySndIdle(A->schn))
	;
    A->running = 0;
}

static void AUNewData(ADesc *A, void *buf, long frames)
...
static long AUStart(ADesc *A, void *buf, long frames)
{   /* Have not actually started the double buffering yet - wait for
	...
	/* DBGFPRINTF((stderr, "AUStart: starting\n")); */
	/* start the command */
	SndPlayDoubleBuffer(A->schn, &(A->dbh));
	A->running = 1;
    }
...
static void AUInBufStartNext(ADesc *A) {
...
static pascal void AUInBufComplete(SPBPtr spb) {
...
static void AUInBufStart(ADesc *A) {
	...
	spb->bufferPtr = (A->bufs[i])->dbSoundData;
    }
	       ...
	       int nchannels, int encoding)
{
  ...
  if (mode == PLAY) {
    ...
    DBGSTR("AInit:play\n");
    DBGSTR("  desbuf="); DBGINT(desbuf); DBGSTR("\n");
	...
	A->bufs[i] = (SndDoubleBuffer *)NewPtr(dbbytes);
	if(A->bufs[i] == NULL)  {
	/*    DBGFPRINTF((stderr, "AUOpenIn: no memory for buffers\n")); */
	    SPBCloseDevice(inRefNum);
	    return -1;
	}
	A->bufs[i]->dbNumFrames = 0;
...
{
  int frames = nSamples/A->nChannels;
  int gotframes = 0;
  
  DBGSTR("ARead: samps="); DBGINT(nSamples); 
  DBGSTR(" curbuf="); DBGINT(A->currentBuf); 
  DBGSTR(" cmpbuf="); DBGINT(A->bufsCompleted); 
  DBGSTR(" issbuf="); DBGINT(A->bufsIssued); DBGSTR("\n");
  ...
  /* Grab buffers until satisfied */
  {
    ...
    while (remain > 0 && A->bufFull[curbuf] != -1) {
      ...
      DBGSTR(" dbnumf="); DBGINT(db->dbNumFrames); DBGSTR("\n");
  ...
  DBGSTR("ARead got: "); DBGINT(gotframes * A->nChannels); DBGSTR(" samps\n");
...
SnackAudioReadable(ADesc *A)
{
  ...
  DBGSTR("ASampsLeft: curbuf="); DBGINT(A->currentBuf);
  DBGSTR(" dbNumF="); DBGINT(db->dbNumFrames);
  DBGSTR(" smpslft="); DBGINT(nsamps); DBGSTR("\n");
...
SnackMixerSetInputJack(Tcl_Interp *interp, char *jack, char *status)
{
  ...
  if (oe == 0) {
    ...
    sbuf[0] = jack+1;  /* 1=MIC, 2=LINE; works on my 2400, but not portable */
