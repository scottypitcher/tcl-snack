	       ...
	       Tcl_Obj *CONST objv[])
{
    ...
    */
	...
	case TOP:
	    ...
	    iir->inputs = (double *) ckalloc( n * sizeof(iir->inputs[0]) );
	...
	case BOTTOM:
	    ...
	    iir->outputs = (double *) ckalloc( n * sizeof(iir->outputs[0]) );
...
iirStartProc(Snack_Filter f, Snack_StreamInfo si)
{
    ...
    for (i = 0; i < iir->nInTaps; i++ ) {
	iir->inputs[i] = 0;
    }
  
    for (i = 0; i < iir->nOutTaps; i++ ) {
	iir->outputs[i] = 0;
    }
    iir->in = iir->out = 0;
	    ...
	    int *inFrames, int *outFrames)
{
	...
	insmp = (double) in[i];
	iir->inputs[iir->in] = insmp;

	/* output is initially zero.  We add up the contribution fo
	   the input history elements */
/* XXX deal with multi-channel here */
	outsmp = 0;
	if (iir->itaps != NULL) {
	    ...
	    */
		...
		outsmp += iir->itaps[j] * iir->inputs[k];
		k = (k+1)%(iir->nInTaps);
	    }
	    iir->in = (iir->in+1)%(iir->nInTaps);
	}
...
/* XXX deal with multi-channel here */
		...
		outsmp -= iir->otaps[j] * iir->outputs[k];
		k = (k+1)%(iir->nInTaps);
	    }
	    iir->out = (iir->out+1)%(iir->nOutTaps);

	    /* here is where we use the zero'th element */
	    outsmp /= iir->otaps[0];
	    iir->outputs[iir->out] = outsmp;
	} 
	out[i] = (float) (outsmp + iir->noise * normalDeviate()
